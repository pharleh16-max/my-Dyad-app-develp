...
export default function AdminEmployeeApproval() {
  ...
  const { data: pendingProfiles, isLoading, error } = useQuery<Profile[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('profiles')
        .select('id, full_name, employee_id, role, status, created_at, auth_users(email)') // Fetch email from auth.users
        .eq('status', 'pending');
      ...
      return data.map(p => ({
        ...p,
        email: (p as any).auth_users?.email, // Access email from the joined table
      })) as Profile[];
    },
  });
  ...
  const handleConfirmAction = () => {
    if (selectedProfile && actionType) {
      updateProfileMutation.mutate({
        ...
        role: selectedProfile.role, // Keep existing role, or set to 'employee' if needed
      });
    }
  };
  ...
  return (
    <>
      <AdminHeader title="Employee Approval" />
      <SideMenu
        isOpen={sideMenuOpen}
